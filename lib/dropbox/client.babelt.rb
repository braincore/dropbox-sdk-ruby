...
{% endif %}
#
# Returns: {{ op.response_segmentation.segments[0].data_type.name|class }}
...
def {{ op.name|method }}({{ required_arg_list(request_segment.all_required_fields)|string_slice(0, -2) }}
    {%- if request_segment.all_required_fields|length > 0 and (op.request_segmentation.segments[1] or request_segment.all_optional_fields|length) -%}
      ,{{ ' ' }}
    {%- endif -%}
    {%- if op.request_segmentation.segments[1] -%}
      {{ op.request_segmentation.segments[1].name|variable }}
